...
export function AcademyStructure() {
  ...
  return <section className="academy-structure" id="structure"><div className="wrap">
    ...
    <div className="structure-level-label"><span>Рівень 1</span><b>Академія</b><small>коренева організація</small></div>
    <div className="structure-root-line"><button type="button" onClick={() => selectBranch("governance")}><span className="structure-root-logo"><img src="/brand/apsvt-official-logo.png" alt="" /></span><span className="structure-root-copy"><small>АПСВТ · 1993</small><b>Академія праці, соціальних відносин і туризму</b></span><span className="structure-root-count"><b>{branches.length}</b><small>напрями структури</small></span></button></div>
    <div className="structure-level-label is-second"><span>Рівень 2</span><b>Напрям</b><small>оберіть одну з чотирьох гілок</small></div>
    <div className="structure-branch-tabs" role="tablist" aria-label="Гілки структури Академії">{branches.map((branch, index) => <button role="tab" aria-selected={branchId === branch.id} aria-controls={`structure-panel-${branch.id}`} className={branchId === branch.id ? "active" : ""} onClick={() => selectBranch(branch.id)} type="button" key={branch.id}><span>{String(index + 1).padStart(2, "0")}</span><b>{branch.label}</b><small>{branch.short}</small><i>{nodes.filter((node) => node.branch === branch.id).length} підрозділів</i></button>)}</div>
    <div className="structure-current-path" aria-live="polite"><span>Академія</span><i>→</i><b>{activeBranch.label}</b><i>→</i><strong>{active.title}</strong></div>
    <div className="structure-layout">
      <div className="structure-unit-panel" id={`structure-panel-${branchId}`} role="tabpanel"><header><span>Рівень 3</span><div><b>{activeBranch.label}</b><small>{branchNodes.length} підрозділів у цій гілці</small></div></header><div className="structure-node-grid">{branchNodes.map((node, index) => <button className={active.id === node.id ? "active" : ""} type="button" onClick={() => setActiveId(node.id)} aria-pressed={active.id === node.id} key={node.id}><span>{String(index + 1).padStart(2, "0")}</span><small>{node.eyebrow}</small><b>{node.title}</b><i>→</i></button>)}</div></div>
      <aside className="structure-detail" aria-live="polite">
        <div className="structure-detail-index">Обраний підрозділ <b>{String(branchNodes.findIndex((node) => node.id === active.id) + 1).padStart(2, "0")}</b></div><small>{active.path}</small><span>{active.eyebrow}</span><h3>{active.title}</h3><p>{active.description}</p><div className="structure-facts">{active.facts.map((fact) => <b key={fact}>{fact}</b>)}</div><Link href={active.href}>{active.linkLabel} →</Link>
      </aside>
    </div>
  </div></section>;
}
