...
export function ScheduleBrowser({ lessons }: { lessons: Lesson[] }) {
  ...
  return <div className="schedule-browser">
    ...
    <div className="schedule-table-wrap"><table className="schedule-table"><thead><tr><th>Дата / день</th><th>Час</th><th>Дисципліна</th><th>Тип</th><th>Курс</th><th>Викладач</th><th>Аудиторія</th></tr></thead><tbody>{filtered.map((lesson) => <tr key={`${lesson.date}-${lesson.time}-${lesson.course}`}><td data-label="Дата / день"><b>{lesson.date}</b><small>{lesson.day}</small></td><td className="time-cell" data-label="Час">{lesson.time}</td><td className="schedule-subject" data-label="Дисципліна"><strong>{lesson.course}</strong><small>{lesson.faculty}</small></td><td data-label="Тип"><span className="lesson-type">{lesson.type}</span></td><td data-label="Курс">{lesson.group}</td><td className="schedule-teacher" data-label="Викладач">{lesson.teacher}</td><td data-label="Аудиторія"><em className={lesson.room === "онлайн" ? "room online" : "room"}>{lesson.room}</em></td></tr>)}</tbody></table></div>
