...
export default async function Page({params}:{params:Promise<{slug:string}>}){
  ...
  return <main id="top"><SiteHeader />
    <section className="program-hero"><div className="program-hero-bg"><img src={program.image} alt="" /></div><div className="wrap program-hero-in"><Link href="/programs" className="back-link">← Усі програми</Link><span className="program-code">{program.code}</span><h1>{program.title}</h1><p>{program.short}</p></div></section><div className="hero-rule" />
    <section><div className="wrap program-intro"><div><div className="idx">01 / Про програму</div><h2>Навчання з практичним результатом</h2><p className="program-lede">{program.overview}</p><div className="focus-list">{program.focus.map((item,i)=><div key={item}><span>0{i+1}</span><b>{item}</b></div>)}</div></div><aside className="program-facts"><div><span>Рівень</span><b>{program.levels}</b></div><div><span>Тривалість бакалаврату</span><b>{program.duration}</b></div><div><span>Денна форма</span><b>{program.price}</b></div><div><span>Заочна форма</span><b>{program.partTimePrice}</b></div>{program.masterPrice&&<div><span>Магістратура, денна</span><b>{program.masterPrice}</b></div>}<small>Вартість для вступників 2026 року, за один навчальний рік.</small><Link className="cta" href="/admissions"><span>Почати вступ</span></Link></aside></div></section>
    <section className="soft"><div className="wrap"><div className="sec-head"><div><div className="idx">02 / Навчальний план</div><h2>Що ви вивчатимете</h2></div><Link className="sec-link" href="/documents/tuition-2026-2027.pdf">Офіційна вартість →</Link></div><div className="study-plan">{program.plan.map((year)=><article key={year.year}><h3>{year.year}</h3><ul>{year.courses.map(course=><li key={course}>{course}</li>)}</ul></article>)}</div><p className="plan-note">Структура показує основні компоненти програми. Повні офіційні плани, силабуси, обов’язкові та вибіркові дисципліни доступні в матеріалах нижче.</p></div></section>
    ...
    <section><div className="wrap"><div className="sec-head"><div><div className="idx">05 / Офіційні матеріали</div><h2>Плани й документи</h2></div></div><div className="official-materials">{program.materials.map(item=><Link href={item.href} key={item.href}><span>PDF / HTML</span><b>{item.label}</b><strong>→</strong></Link>)}</div></div></section>
    <SiteFooter /></main>;
}
