...
export function ProgramFinder({ index = "02 / Тест на програму" }: { index?: string }) {
  ...
  return <section id="test" className="finder-section"><div className="wrap"><div className="idx">{index}</div><div className="finder-shell">
    {step < questions.length ? <>
      ...
      <div className="finder-results">{result.map(({ program, fit }, index) => <Link href={program.slug === "law" ? "/departments/law-faculty#faculty-programmes" : `/programs/${program.slug}`} key={program.slug}><span>0{index + 1}</span><div><b>{program.title}</b><small>{program.short}</small></div><em>{fit}% збіг</em><strong>→</strong></Link>)}</div>
