    ...
    {groups.map((group, groupIndex) => {
      ...
      return <section className={`department-public-section department-public-${group.type}`} id={`department-${group.type}`} key={group.type}>
        <div className="wrap">
          ...
          {group.type === "teacher" && <div className="department-teacher-grid">{items.map((entry) => <article key={entry.id}><div className="department-teacher-photo faculty-portrait-surface">{entry.imageUrl ? <img src={entry.imageUrl} alt={entry.imageAlt || entry.title} /> : <div className="department-teacher-placeholder">{entry.title.split(/\s+/).map((part) => part[0]).slice(0, 2).join("")}</div>}</div><div><small>{entry.role}</small><h3>{entry.title}</h3><p>{entry.summary}</p><nav>{entry.email && <a href={`mailto:${entry.email}`}>Email ↗</a>}{entry.profileUrl && <a href={entry.profileUrl} target="_blank" rel="noreferrer">Науковий профіль ↗</a>}</nav></div></article>)}</div>}
