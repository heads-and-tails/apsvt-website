...
export function SiteHeader(){const pathname=usePathname();const [open,setOpen]=useState(false);return <header className="legacy-header"><div className="wrap topbar">
  ...
  <div className="hdr-right"><Link className="lang-toggle" href="/en" aria-label="English version">EN</Link><Link className="cta hdr-cta" href="/admissions"><span>Вступ 2026</span></Link><button className={`burger ${open?"active":""}`} onClick={()=>setOpen(!open)} aria-label="Меню" aria-expanded={open}><i/><i/><i/></button></div>
  </div></header>}
