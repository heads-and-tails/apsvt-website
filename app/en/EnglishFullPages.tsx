...
export function EnglishPeoplePage() {
  return <main id="top" className="english-page"><SiteHeader />
    ...
    <section><div className="wrap"><div className="sec-head"><div><div className="idx">01 / Leadership</div><h2>Rectorate</h2></div><Link className="sec-link" href="/en/research">Research profiles →</Link></div><div className="leadership-grid">{leadership.map((person) => <article className="leader-card" key={person.name}><div className="leader-photo">{person.img ? <img src={person.img} alt={person.name} /> : <div className="photo-placeholder"><span>{person.initials}</span><small>Photo forthcoming</small></div>}<span className="leader-role">{person.role}</span></div><div className="leader-info"><h3>{person.name}</h3><p>{person.detail}</p>{person.profile && <a href={person.profile} target="_blank" rel="noreferrer">Google Scholar ↗</a>}</div></article>)}</div></div></section>
...
export function EnglishMaterialsPage() {
  return <main id="top" className="english-page"><SiteHeader />
    ...
    <section><div className="wrap"><div className="materials-language-note"><span>Complete catalogue</span><p>The interface is in English. Official documents retain their original approved language and Visnyk journal entries open the corresponding official issue.</p></div><MaterialsBrowser language="en" /></div></section>
    <EnglishFooter />
  </main>;
}
