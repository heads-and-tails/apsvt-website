...
export async function POST(request:Request){try{const publisher=await requirePagePublisher("/news");const body:unknown=await request.json();if(!valid(body))return NextResponse.json({error:"Заповніть усі обов’язкові поля"},{status:400});const post=await createPost(body,publisher.email);return NextResponse.json(post,{status:201,headers:{"Cache-Control":"private, no-store"}})}catch(error){const denied=error instanceof Error&&(error.message==="UNAUTHORIZED"||error.message==="FORBIDDEN_SCOPE");console.error("POST /api/posts failed",error);return NextResponse.json({error:denied?"Доступ заборонено":"Не вдалося створити матеріал. Спробуйте ще раз."},{status:denied?403:500,headers:{"Cache-Control":"private, no-store"}})}}
