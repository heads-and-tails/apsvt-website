...
export async function PATCH(request:Request,{params}:Context){try{const publisher=await requirePagePublisher("/news");const body:unknown=await request.json();if(!valid(body))return NextResponse.json({error:"Некоректні дані"},{status:400});const {id}=await params;const post=await updatePost(id,body,publisher.email);return post?NextResponse.json(post):NextResponse.json({error:"Матеріал не знайдено"},{status:404})}catch{return NextResponse.json({error:"Не вдалося зберегти зміни"},{status:403})}}
export async function DELETE(_:Request,{params}:Context){try{await requirePagePublisher("/news");const {id}=await params;await deletePost(id);return NextResponse.json({ok:true})}catch{return NextResponse.json({error:"Не вдалося видалити матеріал"},{status:403})}}
