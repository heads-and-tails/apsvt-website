...
export function WorkspaceDashboard({ initialItems, publisher }: { initialItems: WorkspaceItem[]; publisher: Publisher | null }) {
  ...
  const canEdit = Boolean(publisher);
  ...
  return <div className="workspace-shell">
    ...
    <main className="workspace-main">
      ...
      <section className="workspace-roadmap" id="roadmap"><div className="workspace-section-head"><div><span>Керування портфелем</span><h2>Напрями й статуси</h2><p>{isAdmin ? "Статус змінюється тут і фіксується централізовано." : publisher ? "Ви можете редагувати деталі; статус змінює адміністратор." : "Відкритий огляд планів, прогресу й наступних кроків Академії."}</p></div>{canEdit ? <button type="button" onClick={startCreate}>+ Додати напрям</button> : <a className="workspace-edit-login" href="/panel/login">Увійти для редагування →</a>}</div>
