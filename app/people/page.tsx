...
export default function Page(){return <main id="top"><SiteHeader />
  ...
  <section><div className="wrap"><div className="sec-head"><div><div className="idx">01 / Керівництво</div><h2>Ректорат</h2></div><Link className="sec-link" href="/research">Наукові профілі →</Link></div><div className="leadership-grid">{leadership.map(person=><article className="leader-card" key={person.name}><div className="leader-photo faculty-portrait-surface">{person.img?<img src={person.img} alt={person.name} />:<div className="photo-placeholder" aria-label={person.name}><span>{person.initials}</span></div>}<span className="leader-role">{person.role}</span></div><div className="leader-info"><h3>{person.name==="Наталія Гончаренко"?<>Наталія <span className="leader-surname">Гончаренко</span></>:person.name}</h3><p>{person.detail}</p>{person.profile&&<a href={person.profile} target="_blank" rel="noreferrer">Google Scholar ↗</a>}</div></article>)}</div></div></section>
